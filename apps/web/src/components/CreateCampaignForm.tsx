...
export default function CreateLoanForm() {
  ...
  const uploadMetadata = async (): Promise<{ campaignURI: string; businessURI?: string }> => {
    ...
    setUploadProgress('Creating loan metadata...');
    ...
    setUploadProgress('Finalizing loan data...');
  ...
  if (isSuccess) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-xl p-8 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Loan Request Created!</h2>
        <p className="text-gray-600 mb-4">Your loan request has been successfully created and is now live.</p>
        <button
  ...
  const steps = [
    ...
    { id: 3, title: 'Loan Details', fields: ['loanAmount', 'fundingPeriodDays'] },
    { id: 4, title: 'Image Upload', fields: ['image'] },
  ];
  ...
  const isStepValid = () => {
    ...
    if (currentStep === 1) {
      ...
      const formFieldsValid = requiredFields.every(field => 
        formData[field as keyof LoanFormData]?.toString().trim()
      );
    ...
    // Step 3: Loan Details validation
    if (currentStep === 3) {
    ...
    // Step 4: Image Upload - Require image to be uploaded for form to be valid
    if (currentStep === 4) {
      const fieldsValid = currentStepData.fields.every(field => 
        field === 'image' ? formData.image : formData[field as keyof LoanFormData]?.toString().trim()
      );
      return fieldsValid;
    }
      ...
      <form onSubmit={handleSubmit} onKeyDown={(e) => {
        ...
        {currentStep === 1 && (
          <>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Loan Purpose (2 lines recommended)
              </label>
              <textarea
                name="title"
              ...
              <p className="text-xs text-gray-500 mt-1">
                Keep it concise - this appears as the description on your loan card
              </p>
              
              {/* Live Preview */}
              {formData.title && formData.businessName && (
                <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-xs font-medium text-gray-700 mb-3">Preview how it appears on your loan card:</p>
                  <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 max-w-sm">
                    ...
                    <div className="text-xs text-gray-400">
                      [Loan progress and details would appear here]
                    </div>
                  </div>
                </div>
              )}
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Help lenders understand your business, your journey, and your vision for the future
              </p>
            </div>
          ...
          <>
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-700">Verify your business performance to build credibility</p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg p-3 border border-primary-200">
                <div className="flex items-center justify-between">
                  ...
                  <button className="btn-primary text-sm px-3 py-1.5 rounded-md">
                    Connect
                  </button>
                </div>
              </div>
              ...
              <div className="mt-4 p-3 bg-green-100 rounded-lg border border-green-200">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-xs text-green-700">
                    <strong>Boost credibility:</strong> Verified business data helps lenders understand your business performance and growth potential.
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-primary-50 border border-primary-200 rounded-lg p-6 mb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-primary-900">Connect Your Socials</h3>
                  <p className="text-sm text-primary-700">Link your social media accounts to showcase your network and community trust</p>
                </div>
              </div>
              ...
              <div className="mt-6 p-4 bg-viridian-50 rounded-lg border border-viridian-200">
                <div className="flex items-start gap-3">
                  <svg className="w-5 h-5 text-viridian-600 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <div className="text-sm">
                    <p className="font-medium text-viridian-900 mb-1">Why connect social accounts?</p>
                    <p className="text-viridian-700">
                      Your social connections demonstrate community trust and business credibility. 
                      Lenders can see your network strength and engagement, which helps validate your loan request.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="text-center text-sm text-gray-500">
              Social connections are optional but recommended for building lender confidence
            </div>
          </>
        )}
        ...
        <div className="flex justify-between pt-6">
          ...
          {currentStep < 4 ? (
            ...
            >
              {isSubmitting || isPending || isConfirming ? 'Creating...' : 'Create Loan Request'}
            </button>
          )}
        </div>
      </form>
